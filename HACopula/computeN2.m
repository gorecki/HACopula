...
% \mathcal{N}^2_{\mathcal{F}}(a, theta) introduced in [Gorecki et al., 2017], where
...
if length(families) == 1
    ...
    switch family
        ...
        case '?'
            N2 = {{family, [-1 1]}};
        otherwise
            error('HACopula:BadInputs', ['HACopulafit::computeN2: Unsupported family - ', family]);
    end
    return;
end
...
elseif  ((sum(strcmp('C',families)) == 1) && (sum(strcmp('12',families)) == 1)) || ...
    ...
    error('HACopula:BadInputs', 'HACopulafit::computeN2: Unsupported family combination in the parameter ''families''');
end
