...
switch familyNames
    ...
    case 'A19'
        ...
        v01 = fastrejectionrnd(v0, expm1(theta1), 1./(theta0.^v0), Fm);
    case 'A20'
        ...
        v01 = fastrejectionrnd(v0, expm1(1), 1./(theta0.^v0), Fm);
        if theta2tau(family1,theta1) >= TAU_NUM_UNSTABLE_A20
            warning('HACopula:sampling', ['rndV0: sampling the family combination (' family0 ',' family1 ') with the child parameter (corresponding to tau = ' num2str(theta2tau(family1,theta1)) ') >= ' num2str(TAU_NUM_UNSTABLE_A20) ' may'...
        ...
        if theta2tau(family1,theta1) >= TAU_NUM_UNSTABLE
            warning('HACopula:sampling', ['rndV0: sampling the child family ' family1 ' with the parameter (corresponding to tau = ' num2str(theta2tau(family1,theta1)) ') >= ' num2str(TAU_NUM_UNSTABLE) ' may'...
    ...
    case 'C19'
        ...
        v01 = fastrejectionrnd(v0, expm1(theta1), exp(v0), Fm);
    case 'C20'
        if theta2tau(family1,theta1) >= TAU_NUM_UNSTABLE
            warning('HACopula:sampling', ['rndV0: sampling the child family ' family1 ' with the parameter (corresponding to tau = ' num2str(theta2tau(family1,theta1)) ') >= ' num2str(TAU_NUM_UNSTABLE) ' may'...
                ' produce a sample that is not uniformly distributed in its univariate margins.']);
    ...
    case 'JJ'
        ...
        if theta2tau(family1,theta1) >= TAU_NUM_UNSTABLE
            warning('HACopula:sampling', ['rndV01: sampling the child family ' family1 ' with the parameter (corresponding to tau = ' num2str(theta2tau(family1,theta1)) ') >= ' num2str(TAU_NUM_UNSTABLE) ' may'...
                ' produce a sample that is not uniformly distributed in its univariate margins.']);
        end
    case '1919'
        v01 = tiltedstablernd(exp(theta1), theta0/theta1, v0);          
    otherwise
        error('HACopula:sampling', ['rndV01: The class combination (' family0 ', ' family1 ') is not supported (as efficient sampling routines have not yet been developed).']);
end
